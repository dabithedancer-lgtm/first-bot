...
class Leaderboard(commands.Cog):
    ...
    async def lb(self, ctx):
        ...
        valid_users = [(uid, u) for uid, u in users.items() if isinstance(u, dict) and 'yen' in u]
        sorted_users = sorted(valid_users, key=lambda x: x[1].get('yen', 0), reverse=True)
        ...
        embed.set_author(name="Top 10 Richest Players", icon_url=ctx.guild.icon.url if ctx.guild.icon else None)

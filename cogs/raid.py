...
class LobbyView(View):
    ...
    async def join(self, button, interaction):
        ...
        embed.set_field_at(1, name=f"👥 Members ({len(lobby['members'])}/{self.max})", value=members_list or "None", inline=False)
    ...
    async def start(self, button, interaction):
        ...
        for uid in lobby['members']:
            ...
            for c in user_cards[:2]:
                base = next((v for v in cards_db.values() if v['name'] == c['name']), None)
                if base:
                    stats = compute_stats(base, c['level'], c.get('aura',0), c.get('equipped_item_id'))
                    team_cards.append({ "name": c['name'], "atk": stats['attack'], "hp": stats['health'] })
        ...
        result_embed.set_author(name="Raid Battle Results", icon_url=interaction.user.display_avatar.url)
...
class Raid(commands.Cog):
    ...
    async def raid_base(self, ctx, action: str = None, *, arg: str = ""):
        if action == "create":
            ...
            # Fuzzy search
            boss = next((b for b in bosses.values() if b['name'].lower() == arg.lower()), None)
            ...
            active_lobbies[code] = { "host": ctx.author.id, "boss": boss, "members": [ctx.author.id] }
            ...
            if boss_stats:
                ...
                embed.add_field(name="Boss Stats", value=stats_text, inline=False)
            
            members_list = "\n".join([f"• <@{uid}>" for uid in active_lobbies[code]['members']])
            embed.add_field(

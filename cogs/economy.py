...
class Economy(commands.Cog):
    ...
    async def bal(self, ctx):
        ...
        if chests:
            chest_text = "\n".join([f"• {k}: `{v}`" for k, v in chests.items() if v > 0])
            if chest_text:
                embed.add_field(name="📦 Chests", value=chest_text, inline=False)
    ...
    async def cd(self, ctx):
        ...
        pull_status = f"`{pulls}/12`" + (f" (Next in: {pull_hours}h {pull_mins}m)" if next_pull > 0 else " (Full!)")
        embed.add_field(
            name="🃏 Pulls",
            value=pull_status,
            inline=False
